...
import { Zap, Info, Star, TrendingUp, Users, Trophy } from 'lucide-react';
import Image from 'next/image';
import TimePeriodToggle from '@/components/ui/TimePeriodToggle';
import WinnerCard from '@/components/winners/WinnerCard';
import { MiniChart, LiveIndicator, StatsCard } from '@/components/ui/Graphics';
import { formatCurrency } from '@/lib/utils';
import Header from '@/components/layout/Header';

export default function WinnerCategoryClient({
  category,
  winners,
  ...
  category: any;
  winners: any[];
  initialPeriod?: 'all-time' | 'last-7-days';
}) {
  const [selectedPeriod, setSelectedPeriod] = useState<'all-time' | 'last-7-days'>(initialPeriod);
  ...
  // Recalculate stats based on selected period
  const filteredWinners = winners;
  const totalPayout = filteredWinners.reduce((sum, winner) => sum + winner.payout, 0);
  const avgStake = filteredWinners.reduce((sum, winner) => sum + winner.stake, 0) / (filteredWinners.length || 1);
  const winRate = filteredWinners.length > 0 ? Math.round((filteredWinners.filter(w => w.payout > w.stake).length / filteredWinners.length) * 100) : 0;
